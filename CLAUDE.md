...
- `sources/northstar-ideas/` — northstar-ideas. Top-scored ideas from recent Northstar runs, pulled directly from the SQLite DB at `~/northstar/.northstar/northstar.sqlite` via the `sqlite3` CLI binary (avoids native build dep). Northstar resolves the DB path as CWD-relative (`./.northstar/`), so the canonical location is inside the user's northstar repo, not under `$HOME`. Filters: most recent N completed runs, score ≥ `minScore`, non-rejected only, sorted by score desc. Disable in CI via `DISABLED_SOURCES=northstar-ideas` (DB is local-only).
